    ...
    <article>
        <div class="grid gap-4 md:grid md:grid-cols-5">
                    ...
                    @if ($show->scheduled_at->isFuture())
                        <div class="relative block mt-4 w-[100px]">
                            <svg class="absolute inset-0 w-full h-full" viewBox="0 0 100 100"
                                ...
                                <path d="M0,50 Q25,0 50,50 T100,50" fill="none" stroke="url(#grad)"
                                    stroke-width="8" />
                                <path d="M0,50 Q25,100 50,50 T100,50" fill="none" stroke="url(#grad)"
                                    stroke-width="8" />
                            </svg>
                            <span
                                class="relative w-full text-center z-10 px-3 py-1 text-xs font-bold text-sky-500 uppercase transform -rotate-12 inline-block">
                                Upcoming
                            </span>
                        </div>
                    @endif
                    ...
                    <div>
                        <h2 class="text-2xl font-bold leading-8 tracking-tight text-gray-900 dark:text-gray-100">
                            ...
                            @else
                                {{ $show->title }}
                            @endif
                            <small class="text-gray-500 dark:text-gray-400 font-normal text-sm ml-1"> @
                                {{ $show->venue_name }}, {{ $show->city }}, {{ $show->country }}</small>
                        </h2>
                    </div>
            ...
            <div class="">
                @if ($show->posterUrl())
                    @php
                        $imageInstance = \Spatie\MediaLibrary\Support\ImageFactory::load($show->posterPath());
                    @endphp

                    <div>
                        <a href="{{ $show->posterUrl() }}" target="_blank" class="show-poster"
                            data-pswp-width="{{ $imageInstance->getWidth() }}"
                            data-pswp-height="{{ $imageInstance->getHeight() }}">
                            <img src="{{ $show->posterUrl() }}"
                                class="flex border-2 border-gray-300 dark:border-gray-700 rounded-lg shadow-md hover:shadow-lg transition duration-300 hover:border-primary-500 dark:hover:border-primary-400"
